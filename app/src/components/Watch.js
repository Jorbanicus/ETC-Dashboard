    ...
    const interval = setInterval(() => {
      ...
      fetch(`http://localhost:3001/?filePath=${encodeURIComponent(filePath)}`) 
        ...
        .then(data => {
          ...
          setIsLoading(Date.now() - lastUpdateTime > 420000);
          console.log('State:', {
            lastValue: newValue,
  ...
  return (
    <div className='flex flex-col md:flex-row bg-black p-1 rounded-md items-center'>
      <div className='flex flex-row space-x-4 border-l-4 rounded border-blue-500 text-white mr-3 pl-2'>
        <div style={{ width: '130px', display: 'flex', flexDirection: 'column', justifyContent: 'center', whiteSpace: 'normal', flexShrink: 0 }}>
          <div className='flex flex-start border-b border-white text-3xl font-bold'>{title}</div>
          <div className='flex flex-start whitespace-nowrap'>{description}</div>
        </div>
        <div className='flex flex-row justify-center items-center space-x-2'>
          <TextDisplay data={lastValue} />
          {isLoading ? <FailIcon /> : 
            (lastValue && Date.now() - lastUpdateTime <= 420000) ? <LiveIcon /> : <FailIcon />}
        </div>
      </div>
    </div>
  );
